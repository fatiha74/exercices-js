...
// les 100 femmes de nationalite francaise
let url = "https://randomuser.me/api/?nat=fr&gender=female&results=100";
...
fetch(url)
  ...
  .then(function (data) {
    ...
    for (const value of data.results) {
      ...
      // card.querySelector('.4').textContent = value.name.last;

      // card.querySelector('img src').textContent= value.name.last;

      // image
      card.querySelector("img").setAttribute("src", value.picture.large);
      ...
      //age
      card.querySelector(".card_age").textContent = `Age: ${value.dob.age}`;
